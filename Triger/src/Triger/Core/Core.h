...
#define TR_ENABLE_ASSERTS
	...
	#define TR_ASSERT(x, ...) { if(!(x)) { TR_ERROR("Assertion Failed: {0}", __VA_ARGS__); TR_DEBUGBREAK(); } }
	#define TR_CORE_ASSERT(x, ...) { if(!(x)) { TR_CORE_ERROR("Assertion Failed: {0}", __VA_ARGS__); TR_DEBUGBREAK(); } }
#else
	#define TR_ASSERT(x, ...)
