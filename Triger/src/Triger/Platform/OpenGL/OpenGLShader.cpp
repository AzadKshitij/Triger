...
namespace Triger
{
		...
		: m_FilePath(filepath)
	{
		...
		auto shaderSources = PreProcess(source);
		{
			...
			TR_CORE_WARN("Shader creation took {0} ms", timer.ElapsedMillis());
		}
	...
	std::unordered_map<GLenum, std::string> OpenGLShader::PreProcess(const std::string &source)
	{
		...
		while (pos != std::string::npos)
		{
			...
			TR_CORE_ASSERT(Utils::ShaderTypeFromString(type), "Invalid shader type specified");
	...
	static bool VerifyProgramLink(GLenum& program)
	{
		...
		if (isLinked == GL_FALSE)
		{
			...
			TR_CORE_ASSERT(false, "[OpenGL] Shader link failure!");
			return false;
		}
		return true;
	}
	...
	void OpenGLShader::CreateProgramForAmd()
	{
		...
		else
		{
			...
			if (linked)
			{
				...
				TR_CORE_ASSERT(formats > 0, "Driver does not support binary format");
				Utils::CreateCacheDirectoryIfNeeded();
				GLint length = 0;
	...
	void OpenGLShader::CompileOpenGLBinariesForAmd(GLenum& program, std::array<uint32_t, 2>& glShadersIDs)
	{
		...
		for (auto&& [stage, spirv] : m_VulkanSPIRV)
		{
			...
			if (isCompiled == GL_FALSE)
			{
				...
				TR_CORE_ERROR("{0}", infoLog.data());
				TR_CORE_ASSERT(false, "[OpenGL] Shader compilation failure!");
				return;
			}
	...
	void OpenGLShader::Reflect(GLenum stage, const std::vector<uint32_t> &shaderData)
	{
		...
		TR_CORE_TRACE("OpenGLShader::Reflect - {0} {1}", Utils::GLShaderStageToString(stage), m_FilePath);
		TR_CORE_TRACE("    {0} uniform buffers", resources.uniform_buffers.size());
		TR_CORE_TRACE("    {0} resources", resources.sampled_images.size());

		TR_CORE_TRACE("Uniform buffers:");
		for (const auto &resource : resources.uniform_buffers)
		{
			...
			TR_CORE_TRACE("    Binding = {0}", binding);
			TR_CORE_TRACE("    Members = {0}", memberCount);
		}
	}
